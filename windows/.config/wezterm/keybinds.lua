...
-- OS検出: Windowsでは ALT、macOSでは SUPER (Cmd) を使用
...
return {
  keys = {
    ...
    -- Pane作成 leader + r or d (従来のキーバインドも維持)
    { key = "d", mods = "LEADER", action = act.SplitVertical({ domain = "CurrentPaneDomain" }) },
    { key = "r", mods = "LEADER", action = act.SplitHorizontal({ domain = "CurrentPaneDomain" }) },
    -- Paneを閉じる
    { key = "l", mods = "CTRL", action = act({ CloseCurrentPane = { confirm = false } }) },
    ...
    -- タブ切替 macOS: Cmd+数字, Windows: ALT+数字
    ...
    { key = "9", mods = mod, action = act.ActivateTab(-1) },
